...
import { 
  ...
  Image, HStack, Button, Link, Icon, Flex 
        ...
        <VStack spacing={12} align="stretch">
          ...
          <VStack spacing={4} textAlign="center">
            ...
            <Text fontSize="lg" color="brand.700" fontWeight="bold">
              {isAr ? "نصوص تربط بين عالم الأرقام وعمق المشاعر" : "Texts bridging the world of numbers and depth of emotions"}
            </Text>
          </VStack>
          ...
          <SimpleGrid columns={{ base: 2, md: 3, lg: 4 }} spacing={6}>
            {bookGroups.map((book) => (
              <MotionBox 
                ...
                whileHover={{ scale: 1.05, bg: "whiteAlpha.300", boxShadow: "2xl" }}
                ...
                <Box>
                  <Box h={{ base: "180px", md: "260px" }} mb={4} overflow="hidden" borderRadius="xl" bg="gray.50" shadow="inner">
                    <Image 
                      ...
                      alt={book.titleAr} h="full" w="full" objectFit="contain" 
                    />
                  </Box>
                  <VStack align="start" spacing={3} w="full">
                    <HStack justify="space-between" w="full">
                      <Heading size="xs" noOfLines={1} color="brand.900">{isAr ? book.titleAr : book.titleEn}</Heading>
                      <Icon as={ArrowIcon} color="brand.500" />
                    </HStack>
                    
                    <Text fontSize="10px" fontWeight="900" color="brand.600" letterSpacing="widest" textTransform="uppercase">
                      {isAr ? "طرق الحصول على العمل:" : "Available via:"}
                    </Text>
                  </VStack>
                </Box>
                ...
                <VStack spacing={2} mt={4} w="full">
                  {book.links[0].url !== "#" ? (
                    book.links.map((link, i) => (
                      <Button 
                        key={i} as={Link} href={link.url} isExternal w="full" size="xs" 
                        colorScheme="teal" variant={i === 0 ? "solid" : "outline"} 
                        leftIcon={<FaBookOpen />} fontSize="10px" py={4}
                      >
                        {link.label}
                      </Button>
                    ))
                    ...
                    <Flex w="full" gap={2}>
                      <Button 
                        as={Link} href="https://www.amazon.com/stores/Shalee-Khalil/author/B0GH8H877J" 
                        isExternal flex="1" size="xs" colorScheme="orange" variant="outline" 
                        leftIcon={<FaAmazon />} fontSize="9px" py={4}
                      >
                        Amazon
                      </Button>
                      <Button 
                        as={Link} href="https://play.google.com/store/books/author?id=Shalee+Khalil" 
                        isExternal flex="1" size="xs" colorScheme="blue" variant="outline" 
                        leftIcon={<FaGooglePlay />} fontSize="9px" py={4}
                      >
                        G-Play
                      </Button>
                    </Flex>
